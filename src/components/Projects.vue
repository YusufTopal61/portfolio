    ...
    <div class="max-w-6xl mx-auto">
      <div class="text-center mb-12">
        <h1 class="text-4xl md:text-5xl font-bold text-white mb-4">
          Mijn <span class="bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Projecten</span>
        </h1>
      ...
      <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
        <div
          ...
          :key="project.title"
          :class="[
            'group bg-slate-800/50 rounded-2xl p-6 hover:bg-slate-700/50 transition-all duration-300 transform hover:scale-105 hover:shadow-2xl border border-slate-700/50 hover:border-purple-500/50',
            project.featured ? 'ring-2 ring-purple-500/30' : ''
          ]"
          :style="{ animationDelay: `${index * 100}ms` }"
        >
          <div v-if="project.featured" class="inline-block bg-gradient-to-r from-purple-600 to-pink-600 text-white text-xs px-3 py-1 rounded-full mb-4">
            ⭐ Featured
          </div>
          ...
          <a
            ...
            class="inline-flex items-center gap-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 text-white px-4 py-2 rounded-lg font-medium transition-all duration-300 transform hover:scale-105"
          >
...
const getTechColor = (tech: string) => {
  const colors: Record<string, string> = {
    ...
    'Spring Boot': 'bg-green-700/20 text-green-300 border-green-700/30',
    'TailwindCSS': 'bg-cyan-500/20 text-cyan-300 border-cyan-500/30',
    'Lit': 'bg-purple-500/20 text-purple-300 border-purple-500/30',
    'OOP': 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30',
    'Game Engine': 'bg-pink-500/20 text-pink-300 border-pink-500/30'
  }
  return colors[tech] || 'bg-purple-500/20 text-purple-300 border-purple-500/30'
}
</script>
